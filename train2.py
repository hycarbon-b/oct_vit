...
def train_epoch(epochs, train_loader, model, criterion, optimizer, scheduler, eval_loader=None):
    wandb.watch(model, log="all", log_freq=15, log_graph=True)
    for epoch in range(epochs):
        ...
        for data, label in tqdm(train_loader):
            ...
            label = label
            allocated = torch.cuda.memory_allocated(torch.device('cuda:0'))
            output = model(data)
            ...
            torch.save(model.state_dict(), 'train2.pt')

        
        print(f"Epoch {epoch+1}: | epoch-MSE: {epoch_loss:.5f}")
        eval(eval_loader, model, criterion)
        if(epoch%100 == 0):
            torch.save(model.state_dict(), f'/code/covid_ckpts/train2_epoch{epoch}.pt')
...
def predict(model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(train_list)):
            ...
            data = data.to(torch.float32).unsqueeze(0).to('cuda:0')
...
valid_loader = DataLoader(dataset = valid_data, batch_size=15, shuffle=True)
...
# 获取模型的分类器部分的输入特征数（通常是4096）
n_inputs = model.classifier[6].in_features
# 创建一个新的全连接层，它的输出是你的类别数（在这个例子中是4）
...
# pretrained_net = torch.load('/code/chen/COVID-Net/xg_vit_model_covid_2d.pt')
